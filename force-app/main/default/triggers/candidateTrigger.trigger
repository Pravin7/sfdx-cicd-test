trigger candidateTrigger on Candidate__c (before insert, after insert, before update, after update) {
    // Check the AsyncValidationFlag to determine if this is coming from the future method
    if (!ValidateAddresses.AsyncValidationFlag) {
        ...
        for (Candidate__c newCand : trigger.new) {
            ...
            if ((Trigger.isInsert) ||
                ...
                if (trigger.isBefore) {
                    newCand.Valid_US_Address__c = false;
        ...
        if ((trigger.isAfter) && (ids.size() > 0)) {
            ...
            //TODO Exercise 10-2: Call the validateAddressSOAP method, passing in the ids variable as the parameter.
